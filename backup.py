...
def count_videos_in_playlist(url):
    ydl = yp.YoutubeDL({})
    with ydl:
...
def actual_down(op1, op2, url, path):
    if check_internet_connection() == False:
        ...
        print("Intenet")
    try:
        total_vids = count_videos_in_playlist(url)
    except:
        pass
    print("passed")
    def progress_hook(d):
        ...
        if d['status'] == 'downloading':
            total_bytes = d.get('_total_bytes_str_')
            downloaded_bytes = d.get('_downloaded_bytes_str_')
            ...
            pperc.configure(text="Download Complete", text_color="#006633")
...
def main_window():
    ...
    def on_enter_setting(event):
       ...
       setting_label_text.place(x=530, y=15)
